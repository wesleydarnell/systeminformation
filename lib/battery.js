  ...
  new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if (_linux) {
        ...
        } else if (fs.existsSync('/sys/class/power_supply/battery/uevent')) {
          battery_path = '/sys/class/power_supply/battery/';
        } else if (fs.existsSync('/sys/class/power_supply/qcom-battmgr-bat/uevent')) {
        ...
        } else if (fs.existsSync('/sys/class/power_supply/qcom-battmgr-usb/online')) {
          acPath = '/sys/class/power_supply/qcom-battmgr-usb/online';
        }
        ...
        if (battery_path) {
          fs.readFile(battery_path + 'uevent', (error, stdout) => {
            if (!error) {
              ...
              const chargeCounterMAh = parseInt('0' + util.getValue(lines, 'POWER_SUPPLY_CHARGE_COUNTER', '='), 10) / 1000.0;
              if (!result.currentCapacity && chargeCounterMAh && result.voltage) {
                result.currentCapacity = Math.floor(chargeCounterMAh * result.voltage);
              }
